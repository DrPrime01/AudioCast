  ...
  const navlinkItems = navlink.map((navlinkItem, i) => {
    return (
      <li key={i} className="navlink-item">
        <a href="#" className="navlink">{navlinkItem}</a>
      </li>
    );
  });
    ...
    <nav className="navbar-container">
      ...
      <div className="searchbar-container">
        <form className="searchbar-form">
            <input type="search" className="searchbar-input" placeholder="Search" />
            <button type="submit" className="searchbar-button"><FontAwesomeIcon icon={faSearch} className="search-icon"/></button>
        </form>
      </div>
    </nav>
  );
